...
namespace ComputerShopDataBase.Implementation
{
    public class RecordServiceDB : IRecordService
    {
        ...
        public RecordServiceDB(ComputerDbContext context) { this.context = context; }

        public void SaveItemPrice(RecordBindingModel model)
        {
            if (File.Exists(model.FileName))
            {
                File.Delete(model.FileName);
            }
            var winword = new Microsoft.Office.Interop.Word.Application(); try
            {
                ...
                font = table.Range.Font; font.Size = 14; font.Name = "Times New Roman";
        ...
        public List<StorageLoadViewModel> GetStorageLoad()
        {
            ...
            context.StorageParts.Include(r => r.Part).ToList(),
            stock => stock, stockPart => stockPart.Storage,
            (stock, stockCompList) => new StorageLoadViewModel
            {
        ...
        public void SaveStorageLoad(RecordBindingModel model)
        {
        ...
        public List<CustomerBookingModel> GetCustomerBooking(RecordBindingModel model)
        {
            return context.Bookings.Include(rec => rec.Customer).Include(rec => rec.Item).Where(rec => rec.DateCreate >= model.DateFrom && rec.DateCreate <= model.DateTo).Select(rec => new CustomerBookingModel
            {
        ...
        public void SaveCustomerBooking(RecordBindingModel model)
        {
            ...
            //вставляем заголовок         
            var phraseTitle = new Phrase("Заказы клиентов", new iTextSharp.text.Font(baseFont, 16, iTextSharp.text.Font.BOLD));
            ...
            PdfPTable table = new PdfPTable(6) { TotalWidth = 800F };
            ...
            //заполняем таблицу      
            var list = GetCustomerBooking(model);
